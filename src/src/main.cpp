...
#if defined(NDEBUG)
# error "Akitocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc7dd322b50158baa18cf8e2f5966c545b35f1f58251d71c61787176d2c737cf2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Akitocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Akitocoin Signed Message:\n";
...
{
    // Akitocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Akitocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Akitocoin: 3.5 days
//static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Akitocoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Akitocoin: 2.5 minutes
//static const int64 nTargetSpacing = 2.5 * 60; // Akitocoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Akitocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Akitocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x35ed13d2cbdb5e5c6a0ceb6e6123cc46b33d1de936dc590cf50b249c31461150");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Disneys Mulan sparks backlash over Xinjiang, Hong Kong";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04bac9c3c8db4bfc9a9bc87408a39c93617bf1dafc436e5e7b6867dc0a60d6315713513b1c2ee215b996f6111b9c3249f2533b1b1857cd5b0ee905614883edbab8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1599569613;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084722519;

        if (fTestNet)
        {
            block.nTime    = 1599569604;
            block.nNonce   = 385885597;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x9fc45c8d280b1f32efc4b4cd8c88be7e472ceef9f0be8e3a2509c108a1e246b1"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xbb, 0xda }; // Akitocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AkitocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AkitocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AkitocoinMiner(CWallet *pwallet)
{
    printf("AkitocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("akitocoin-miner");
    ...
    try { loop {
        ...
        printf("Running AkitocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AkitocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AkitocoinMiner, pwallet));
}
